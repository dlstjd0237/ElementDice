...
public class PoolManagerEditor : Editor
{
    const string INFO = "풀링한 오브젝트에 다음을 적으세요 \n void OnDisable()\n{\n" +
        ...
        " * Poolling 할 오브젝트에는 반드시 PoolableMono를 상속 시켜줘야한다.\n" +
        " * Prefab으로 뽑은 오브젝트는 0.Wrok 폴더에 있는 리소스 폴더 안에있는 PoolListSO에 넣어줘야한다.";
...
public class PoolManager : MonoSingleton<PoolManager>
{
    ...
    [SerializeField]
    private string _mixerPath = "PoolListSO";
    private Pool[] _pools;
    ...
    private void Start()
    {
        _pools = Resources.Load<PoolListSO>(_mixerPath).list.ToArray();
